...
for _ in range(k):
    a,b = map(int,input().split()) # 사과위치
    data[a][b] = 1
l = int(input()) # l 방향변화 개수
dir_info = []
for _ in range(l):
...
# def simulate():    
#     x, y = 1, 1
#     data[x][y] = 2 # 뱀이 존재하는 위치를 2로 표시하는게 아이디어다. 뱀 몸통 부딪혀도 게임오바이므로
#     direction = 0
#     time = 0
#     index = 0
...
#             if data[nx][ny] == 0:
#                 data[nx][ny] = 2
#                 q.append((nx,ny)) # 큐도 그냥 이렇게 쓰면 된다
